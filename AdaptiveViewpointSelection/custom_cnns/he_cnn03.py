...
now = dt.datetime.now()
shortnote = '_BN_init' # start with underscore
...
BN_EPS = 1e-7
USE_PRELU = True
data_augmentation = False # use manually augmented data
...
# Conv5
model.add(Convolution2D(128, 7, 7, init=weight_init, border_mode='same',
                      W_regularizer=l2(REG_W_CNN), b_regularizer=l2(REG_B_CNN)))
model.add(BatchNormalization(epsilon=BN_EPS, mode=0, axis=3,
                        momentum=0.99, beta_init='zero', gamma_init='one'))
if USE_PRELU:
